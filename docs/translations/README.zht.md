...
TL;DR：OpenTaint 是 *Semgrep Pro* 和 *CodeQL* 的開源替代方案——一個可自訂、可自行託管的形式化過程間污點分析引擎，專為讓 AI 代理驅動你的安全分析而打造，無需在每次掃描時消耗 token。

AI 產生生產程式碼的速度已經超過了安全團隊能夠跟上的節奏，而為捕捉其錯誤而生的兩類工具，各自都帶來了糟糕的取捨：

- **模式匹配器**（Semgrep OSS、ast-grep、各類 linter）免費且快速，但它們匹配的是語法，而非資料流——跨越函式邊界或持久層的不可信輸入會直接溜過去。真正能捕捉這類問題的、更深入的過程間分析，長期以來一直被鎖在專有工具之中。
...
- **發現模式匹配器遺漏的問題。** 形式化的過程間資料流引擎能夠跨函式邊界、持久層、別名和非同步程式碼追蹤不可信資料。
