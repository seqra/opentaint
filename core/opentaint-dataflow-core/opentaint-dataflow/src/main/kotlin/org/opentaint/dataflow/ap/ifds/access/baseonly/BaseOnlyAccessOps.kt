...
object BaseOnlyAccessOps {
    ...
    fun clearAllOccurrences(
        access: BaseOnlyAccess,
        idx: AccessorIdx,
        keepStartAccessor: Boolean,
    ): BaseOnlyAccess? {
        if (access.suffixIdx != idx) return access
        if (!keepStartAccessor) return null
        return if (access.pastFirst) null else access
    }
