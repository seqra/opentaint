...
private class TraceActionCollector(
    ...
    fun collect(): ActionableRulesCollectionResult {
        ...
        logger.debug {
            "Rule search skipped $unchangedTaintMarkNodes unchanged-mark and " +
                "$coveredZeroStartNodes covered-Zero " +
    ...
    private fun evaluateFullTrace(
        ...
        for ((entryId, entry) in trace.entries.withIndex()) {
            ...
            if (entry is TraceEntry.Action) {
                when (val result = evaluateActionVariants(trace, entry, entryId)) {
    ...
    private fun evaluateActionVariants(
        ...
        for (variant in variants) {
            ...
            val summary = variant.relevantSummary()
            if (summary != null) {
                when (val nestedResult = evaluateSummary(summary)) {
    ...
    private fun ActionVariant.relevantSummary(): SummaryTrace? =
