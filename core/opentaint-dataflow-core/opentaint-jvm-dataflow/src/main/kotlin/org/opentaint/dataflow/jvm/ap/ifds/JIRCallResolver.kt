...
class JIRCallResolver(
    ...
    private fun inheritedInterfaceOverrides(method: JIRMethod, baseClass: JIRClassOrInterface): List<JIRMethod> {
        if (!method.isAbstract) return emptyList()
        if (!baseClass.isInterface) return emptyList()

        val declaringClass = method.enclosingClass
        if (declaringClass == baseClass) return emptyList()
        if (!declaringClass.isInterface) return emptyList()
        if (declaringClass.declaration.location.id !in knownLocationIds) return emptyList()

        return methodOverrides(method, declaringClass)
    }
    ...
    private fun resolveVirtualMethod(
        ...
        for ((method, constraint) in classMethods) {
            ...
            val overrides = methodOverrides(method, constraint.type)
                .ifEmpty { inheritedInterfaceOverrides(method, constraint.type) }
                .filter {
