...
class MayAndMustRelationTest : BasicTestUtils() {
    ...
    fun `check must alias inclusion in may alias`() {
        ...
        val sampleClassNames = allClasses.filter { it.startsWith(ALIAS_SAMPLE_PKG) }
        val sampleClasses = sampleClassNames.map { findClass(it) }.filter { !it.isInterface && !it.isAbstract }
        val methods = sampleClasses.flatMap { it.declaredMethods }
