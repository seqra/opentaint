...
class OwaspBenchmarkTest : AnalysisTest() {
    ...
    // ═══ xss (CWE-79, 40) — ALL @Disabled: return-value-sink engine gap (inv 35), ESCALATED ═══
    // The XSS sink is the returned HTTP response body: `RESPONSE += f'...{bar}...'` then a bare
    // `return RESPONSE` (no render_template_string / make_response / .write CALL to hook). A structural
    // sink `return $A` lowers the source but emits ZERO sinks — a return is a MethodExit edge and
    // PythonTaintRuleGeneration errors ("Non method call sinks are not supported yet"); sinks fire only
    // at calls/attribute reads. Verified via PythonRuleEmitTest `return-value sink emits no sink`
    // (sources=1, sinks=0). Enable this block when return-value-sink support lands, then triage the
    // FALSE half (escape_for_html / html.escape / markupsafe.escape sanitizers). Full detail: inv 35.
    @Disabled("engine gap (inv 35): CWE-79 XSS sink is the returned response body (RESPONSE += f\"...{bar}...\"; return RESPONSE) — a bare return, not a call; return-value sinks emit zero sinks (verified) and fire nowhere")
