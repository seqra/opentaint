...
state at a fan-out join; none is read-only (use verify.py for checks). Run with uv
from the project root: `uv run scripts/generate.py <cmd>`.
  ...
  findings <sarif>    SARIF report -> per-rule finding tracking files (idempotent)
"""
import argparse
import glob
import hashlib
...
from _common import (APPROX, DROPPED, FINDINGS_TR, MODEL, RULES_TR, TRACKING,
                     class_of, classified_keys, dropped_entries, dump_yaml,
                     fqn_base, ledger_verdicted_keys, load_yaml, member_key,
...
def main():
    ...
    f = sub.add_parser("findings", help="seed per-rule finding files from a SARIF report")
    f.add_argument("sarif", help="path to report.sarif")
