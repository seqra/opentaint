...
Not a runnable script — imported by the PEP723 entry points (verify.py, generate.py),
...
# a method counts classified once it sits in any batch classification bucket,
# in build.done, or in skipped.yaml (methods/engine_issues)
...
def classified_keys():
    ...
    for p in batch_files():
        ...
        for item in (doc.get("build") or {}).get("done", []) or []:
            if str(item).strip():
                out.add(member_key(item))
    # methods parked in the merged skipped.yaml also count as classified
    sk = load_yaml(APPROX / "skipped.yaml", {}) or {}
    for key in ("methods", "engine_issues"):
