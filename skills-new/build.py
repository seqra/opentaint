...
For each `<src>/<skill>/main.md.j2` it renders `<out>/<skill>/SKILL.md`, then copies
that skill's `references/` and `scripts/` verbatim. Every leaf skill MUST expose the two
fixed contract sections `sections/input.md` and `sections/output.md`; the build fails loudly
otherwise. Default `--out` is non-destructive (skills-new/dist); point it at `skills` to
replace the real tree.

Jinja2 default delimiters; includes resolve relative to each skill dir (and the src root,
so a future `_base/` inheritance base is reachable).
"""
import argparse
import shutil
import sys
