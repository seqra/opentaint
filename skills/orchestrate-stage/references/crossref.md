...
- overmatch — the trace reproduces, but the same rule also fires on flows that are not this finding: still `reproduced`, and note the overmatch in `notes` so the controls stage has the evidence
...
- no source or sink fact on the expected path — `status: unreproduced`, `cause: rule`. Say in `notes` which side is missing and why. `generate.py controls` turns that into a false-negative target, so report it to MAIN: the controls stage fixes it and the run comes back here. When the run was bootstrapped with controls off, it stays an unreproduced finding with the rule cause as its blocker
...
- the reusable artifacts the run produced: source rules, sink rules, joins, controls, approximations
