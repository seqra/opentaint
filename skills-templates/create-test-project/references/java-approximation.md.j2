...
`opentaint test approximation init <project>` scaffolds the Gradle build plus the test-util jar, `Taint.java`, and the fixed `approximation-rule.yaml` the harness applies. Pass each lib at the exact pinned version from the batch's `dependencies`: this Gradle build is the approximation's own compile environment, so it must recompile from these pins even after the main project drops that dependency.
